...
@app.route('/obtener_frase/<mood>')
def obtener_frase(mood):
    opciones = FRASES.get(mood, ["¡Que tengas un excelente día!"])
...
def enviar_mensaje():
    ...
    if tipo == 'usuario':
        cursor.execute('INSERT INTO mensajes (emisor_id, receptor_id, contenido, fecha) VALUES (?, ?, ?, ?)',
                       (session['usuario_id'], target_id, contenido, fecha_actual))
    else:
        cursor.execute('INSERT INTO mensajes (emisor_id, grupo_id, contenido, fecha) VALUES (?, ?, ?, ?)',
                       (session['usuario_id'], target_id, contenido, fecha_actual))
